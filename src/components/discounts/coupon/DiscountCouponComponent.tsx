...
const DiscountCouponComponent: React.FC<DiscountCouponComponentProps> = ({
  ...
  const [isReserved, setIsReserved] = useState<boolean>(
    discountCoupon.status.status === "RESERVED"
  );
  const [isUsed, setIsUsed] = useState<boolean>(
    discountCoupon.status.status === "USED"
  );
  ...
  const onReserveHandler = () => {
    ReserveDiscountCoupon(discountCoupon, discountsType).then(
      (updatedDiscountCoupon) => {
  ...
  const onMakePublicHandler = () => {
    MakePublicDiscountCoupon(discountCoupon, discountsType).then(
      (updatedDiscountCoupon) => {
  ...
  const onUndoReservationHandler = () => {
    UndoReserveDiscountCoupon(discountCoupon, discountsType).then(
      (updatedDiscountCoupon) => {
        selfDestroyer(updatedDiscountCoupon);
      }
    );
  ...
  const onConfirmationHandler = (confirmationResponse: boolean) => {
    if (confirmationResponse)
      UseUpDiscountCoupon(discountCoupon, discountsType).then(
        (updatedDiscountCoupon) => {
    ...
    <Fade in={!invisible}>
      <Card>
        ...
        {((isReserved && discountsType !== "reserved") || isUsed) && (
          <UserReservedComponent
            reserved={isReserved && discountsType !== "reserved"}
            used={isUsed}
            user={
              discountCoupon.status.reserved_by ||
              discountCoupon.status.used_by ||
              ""
            }
          />
        )}
        {showConfirmation && (
          <CouponConfirmationComponent
            confirmationLiftUp={onConfirmationHandler}
            verification={tryUse}
          />
        )}
        ...
        <CardActions>
          ...
          >
            <Accordion expanded={informationIsOpen}>
              <AccordionSummary />
              <AccordionDetails>
                <CouponInformationComponent
                  couponCard={discountCoupon.discount_card}
          ...
          {discountsType !== "reserved" && (
            <Fab
              ...
              disabled={
                (isReserved && discountsType !== "reserved") ||
                isUsed ||
                showConfirmation
              }
            >
          ...
          {discountsType === "reserved" && (
            <Fab
              onClick={onUndoReservationHandler}
              disabled={
                (isReserved && discountsType !== "reserved") ||
                isUsed ||
                showConfirmation
