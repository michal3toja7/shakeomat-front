...
const ControlDiscountAppComponent: React.FC<
  ControlPanelComponentProps
> = () => {
  const [contentSwitchers, setContentSwitchers] =
    useState<IContentSwitcher[]>(initContentSwitcher);
        ...
        >
          <Tabs value={isSelected} centered onChange={contentSwitcherHandler}>
            {contentSwitchers.map((contentSwitcher) => {
              return (
                <Tab
                  value={contentSwitcher.key}
                  key={contentSwitcher.key}
