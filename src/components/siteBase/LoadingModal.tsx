...
const LoadingModalItem: React.FC<LoadingModalItemProps> = ({
  isLoading,
  loadingMassage,
}: LoadingModalItemProps) => {
  ...
  useEffect(() => {
    ...
    if (isLoading && isLoading !== loading) setLoading(isLoading);
  }, [isLoading]);
  return (
    <div
      className={`${style["loader-container"]} ${
        isLoading && style["show-modal"]
    ...
    >
      ...
      {loadingMassage && (
        <div className={"load-description"}>{loadingMassage}</div>
      )}
    </div>
  );
};
